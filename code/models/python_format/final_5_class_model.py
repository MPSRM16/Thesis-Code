# -*- coding: utf-8 -*-
"""final_5_class_model.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ih0CyhsA5xIunobMb8pbSL6AKF7aE6K4
...
| `pepmass_range`            | Normalization bounds for parent ion mass: `{'min': 500.0, 'max': 6000.0}` |
...
3. **Label Assignment**: Reads spectrum titles to determine PTM class (label 0–4).
...
If the evaluation score improves and the batch, the model is saved with a timestamp and score in the filename for version tracking.
...
while True:
    ...
    try:
      feature_batch = combine_features(
          ...
          pepmass_range,
          num_bins,
          window_normaliation_size,
          assume_observed=assume_observed
          )
    except RuntimeError as e:
        print(f"⚠️ Skipping batch due to sanity check failure: {e}")
        continue
    ...
    if (not math.isnan(score) and score > best_score and score >= min_score_threshold):
        best_score = score
